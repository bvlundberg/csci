...
-- normalize a ghci
-- list, i.e., sort and remove (now adjacent) duplicates
norm :: Ord a => [a] -> [a]
...
unionFSM m1@(qs1, s1, fs1, ts1) m2@(qs2, s2, fs2, ts2) = ((qs1 >< qs2), (s1, s2), norm [(q1, f2) |q1 <- qs1, f2 <- fs2] ++ [(f1,q2) | f1 <- fs1, q2 <- qs2], [((q1,q2), z, ((delta m1 q1 z), (delta m2 q2 z)))| q1 <- qs1, q2 <- qs2, z <- sigma])
